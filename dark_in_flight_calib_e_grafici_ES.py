...
for i in range(21,len(file)):			       ##  per ogni i che va da 21(?) alla fine  (immagino parliamo del R+alto)		
    filen=file[i]
    print(filen)
    ...
    f750=np.dstack((f750,matf750))	          ##  stocca dentro f750 tutte le immagini a R+alto  (NON CREDO SERVA NULLA)
...
#   img a cui viene sottratta l'ultima immagine di ogni serie di IT
    ...
    u750=np.dstack((u750,ultima750))         ##  stocca in u750 l'ultima immagine a R+alto
...
for i in range(0,20): #len(file)
    ...
    wx = data['img_WX'][:,:,1:11]
    WX=np.dstack((WX,wx))
    matf750 = data['img_cfilt'][:,:,1:11]                           ##  stocca dentro f750 tutte le immagini a R+basso
    ...
    zval=len(wx[0,0,:])
    calWX=np.zeros((64,128,10))
    for jj in range(0,zval):
        calWX[:,:,jj] = wx[:,:,jj]-uWX[:,:,i+1]
   # breakpoint()
   # calWX=wx-np.repeat(uWX[:,:,i+1].reshape(64,128,1),zval,axis=2)      ## a me fa orrore comunque 
                                                                         ## è la wx (64x128x10) - la  ultima immagine periodizzata sull'asse z
                                                                         ## zval immagino sia sempre 10																		
   # calWX=wx-np.repeat(uWX[:,:,i+1],zval,axis=2)       
    meanWX=np.average(calWX)
    meanWX=np.zeros(10)  
    #meanwx=np.zeros(1)
    for j in range(0,zval):
        meanWX[j]=np.average(calWX[:,:,j])
        #print(j)
    # breakpoint()
    vval=len(meanWX)
    ...
    for u in range(0,zval):
        cal920[:,:,u]=matf920[:,:,u]-u920[:,:,i+1]-meanWX[u]
        cal550[:,:,u]=matf550[:,:,u]-u550[:,:,i+1]-meanWX[u]
        cal420[:,:,u]=matf420[:,:,u]-u420[:,:,i+1]-meanWX[u]
        cal750[:,:,u]=matf750[:,:,u]-u750[:,:,i+1]-meanWX[u]
    ...
    f_IT=np.array(data['IT'][0:len(data['img_cfilt'][0,0,1:11])],dtype=float)
...
#         Z3=matf920[:,:,pro]-u920[:,:,i+1]-meanWX[pro]                                                     ## qui visualizza  cal920[:,:,u] e basta 
# 		                                                                                            ##  sono già sia calibrate che mitigate no?
#         mini=np.min((Z3))
#         maxi=np.max((Z3))  
#         #print(mini,maxi)
...
#         ind_4=10*(i)+1                                                                               ## il primo set di immagini è storato in 
#                                                                                                    ## cal920_2b(:,:,1:10)
# 																								   ## perchè cal920_2b(:,:,0)  è di 0
# 																								   ## quindi se vuoi visulaizzare la 3a del blocco i
# 																								   ## cal920_2b(:,:,10*(i-1)+3)
# 																								   
 		
#         Z4=cal920_2b[:,:,ind_4+pro]
#         mini=np.min((Z4))
#         maxi=np.max((Z4))  
#         #print(mini,maxi)
...
#%%
#%%immagini raw di ciascuna serie di IT e la media della WX 
